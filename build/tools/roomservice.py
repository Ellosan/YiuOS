...
if not depsonly:
    print(
        f'Device {device} not found. Attempting to retrieve device repository from YiuOS Github (http://github.com/LineageOS).'
    )
...
if not depsonly:
    githubreq = urllib.request.Request(
        'https://raw.githubusercontent.com/YiuOS/mirror/main/default.xml'
    )
    try:
...
def is_in_manifest(tag, attr, attr_value):
    ...
    # ... and don't forget the yiuos snippet
    try:
        lm = ElementTree.parse('.repo/manifests/snippets/yiuos.xml')
        lm = lm.getroot()
    except Exception:
        lm = ElementTree.Element('manifest')
    ...
    for dependency in dependencies:
        ...
        if dependency_type == 'kernel':
            ...
            if is_in_manifest('project', 'path', repo_target):
                print(f'YiuOS/{repo_name} already fetched to {repo_target}')
                continue
...
print(
    f'Repository for {device} not found in the YiuOS Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml.'
)
