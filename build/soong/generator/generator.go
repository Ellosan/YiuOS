...
func init() {
	android.RegisterModuleType("yiuos_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = yiuosExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = yiuosExpandVariables(ctx, dep_file)
	...
	cmd := yiuosExpandVariables(ctx, String(g.properties.Cmd))
