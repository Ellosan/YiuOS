...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^yiuos_") ; then
        YIUOS_BUILD=$(echo -n $1 | sed -e 's/^yiuos_//g')
    else
        YIUOS_BUILD=
    fi
    export YIUOS_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/yiuos/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the YiuOS model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch yiuos_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/yiuos-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.yiuos.device | grep -q "$YIUOS_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $YIUOS_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function yiuosremote()
{
    ...
    git remote rm yiuos 2> /dev/null
    ...
    local YIUOS_USER=$(git config --get review.review.lineageos.org.username)
    if [ -z "$YIUOS_USER" ]
    then
        git remote add yiuos ssh://review.lineageos.org:29418/$PFX$PROJECT
    else
        git remote add yiuos ssh://$YIUOS_USER@review.lineageos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'yiuos' created"
}
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.yiuos.device | grep -q "$YIUOS_BUILD");
    then
...
function yiuosrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "YiuOS Gerrit Rebase Usage: "
        echo "      yiuosrebase <path to project> <patch IDs on Gerrit>"
...
function dopush()
{
    ...
    if (adb shell getprop ro.yiuos.device | grep -q "$YIUOS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/yiuos/scripts/repopick/repopick.py "$@"
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $YIUOS_FIXUP_COMMON_OUT ]; then
...
function build_kernel() {
    ...
    local yiuos_version="yiuos-$(_get_build_var_cached PRODUCT_VERSION_MAJOR).$(_get_build_var_cached PRODUCT_VERSION_MINOR)"
    ...
    if [[ "${SKIP_KERNEL_SYNC}" != "true" && "${SKIP_KERNEL_SYNC}" != "1" ]]; then
        ...
        local repo_init_args=("-b" "${yiuos_version}")
        if [ -n "${YIUOS_MIRROR}" ]; then
            repo_init_args+=("--reference" "${YIUOS_MIRROR}")
        fi
